...
-(void)hideToast
{
    ...
    if(!toast)
    {
        //DDLogDebug(@"fuck :%@",[[UIApplication sharedApplication] windows]);
...
- (CGPoint)getPositionFor:(id)point toast:(UIView *)toast {
    ...
    //DDLogDebug(@"Error: Invalid position for toast.");
...
- (UIView *)makeViewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image isShadow:(BOOL)_isShadow{
    ...
    if (message != nil) {
        ...
        [messageLabel setFrame:CGRectMake(0.0, 0.0, expectedSizeMessage.width+40, expectedSizeMessage.height+20)];
    }
